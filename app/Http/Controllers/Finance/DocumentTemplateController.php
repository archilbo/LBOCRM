...
class DocumentTemplateController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('Finance/Templates/Index', [
            ...
            'routes' => [
                ...
                'resetQuote' => route('finance.templates.reset', 'quote'),
                'resetInvoice' => route('finance.templates.reset', 'invoice'),
                'resetReceipt' => route('finance.templates.reset', 'receipt'),
            ],
        ]);
    }
    ...
    public function store(StoreDocumentTemplateRequest $request): RedirectResponse
    {
        ...
        $source = !empty($data['copy_from_id']) ? FinanceTemplate::find($data['copy_from_id']) : null;
        ...
        $template = DB::transaction(function () use ($payload) {
            ...
            if ($payload['is_default']) {
                FinanceTemplate::where('type', $payload['type'])->update(['is_default' => false]);
            }
    ...
    public function update(UpdateDocumentTemplateRequest $request, FinanceTemplate $documentTemplate): RedirectResponse
    {
        ...
        DB::transaction(function () use ($data, $documentTemplate) {
            ...
            if ($data['is_default']) {
                FinanceTemplate::where('type', $documentTemplate->type)
    ...
    {
        if ($documentTemplate->is_default && FinanceTemplate::where('type', $documentTemplate->type)->where('is_default', true)->count() <= 1) {
    ...
    public function setDefault(FinanceTemplate $documentTemplate): RedirectResponse
    {
        DB::transaction(function () use ($documentTemplate) {
            FinanceTemplate::where('type', $documentTemplate->type)->update(['is_default' => false]);
    ...
    public function resetDefault(string $type): RedirectResponse
    {
        ...
        DB::transaction(function () use ($type) {
            FinanceTemplate::where('type', $type)->update(['is_default' => false]);
    ...
    private function sampleData(): array
    {
        ...
        return [
            ...
            'items' => [
                ['position' => 1, 'title' => 'Etudes architecturales', 'description' => 'Plans et dossier administratif', 'quantity' => 1, 'unit' => 'forfait', 'unit_price_display' => '8 000.00 ' . $currency, 'discount_rate' => 0, 'tva_rate' => 20, 'total_ht_display' => '8 000.00 ' . $currency, 'total_tva_display' => '1 600.00 ' . $currency, 'total_ttc_display' => '9 600.00 ' . $currency],
                ['position' => 2, 'title' => 'Suivi dossier', 'description' => 'Suivi administratif', 'quantity' => 1, 'unit' => 'forfait', 'unit_price_display' => '2 000.00 ' . $currency, 'discount_rate' => 0, 'tva_rate' => 20, 'total_ht_display' => '2 000.00 ' . $currency, 'total_tva_display' => '400.00 ' . $currency, 'total_ttc_display' => '2 400.00 ' . $currency],
            ],
