...
import {
    dossierWorkflowOptions,
    getDossierReadiness,
    getDossierStatusClass,
    getDossierWorkflowLabel,
...
function ProjectDetailPanel({
    ...
    const items = getDossierReadiness(dossier);
        ...
        <aside className="crm-panel overflow-hidden">
            <div className="border-b border-[var(--crm-border)] p-4">
                ...
                <div className="mt-2 flex items-start justify-between gap-3">
                    ...
                    <span className={`shrink-0 rounded-full border px-2 py-1 text-[11px] font-semibold ${getDossierStatusClass(dossier.status)}`}>
                        {dossier.status}
                    </span>
                </div>
            </div>
            ...
            <div className="space-y-5 p-5">
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="crm-panel-soft p-3">
                        ...
                        <p className="mt-1 truncate text-sm font-semibold text-[var(--crm-gold)]">{getDossierWorkflowLabel(dossier.workflowStep)}</p>
...
export default function DossiersIndex({ dossiers, locationGroups, clients, metrics }: PageProps) {
    ...
    const workflowOptions = useMemo(() => [
        ...
        ...dossierWorkflowOptions.map((option) => ({
            ...option,
            count: countByValue(dossiers, (dossier) => dossier.workflowStep, option.id),
        })),
    ], [dossiers]);
                                ...
                                <table className="crm-table min-w-[980px]">
                                    ...
                                    <tbody>
                                        {pagedDossiers.map((dossier) => {
                                            const done = getDossierReadiness(dossier).filter((item) => item.done).length;
                                                    ...
                                                    <td>
                                                        <div className="w-[140px]">
                                                            <div className="mb-1 flex justify-between text-xs text-[var(--crm-text-muted)]">
                                                                <span>{done}/6</span>
                                                                <span className={`rounded-full border px-2 py-0.5 ${getDossierStatusClass(dossier.status)}`}>{dossier.status}</span>
                                                            </div>
