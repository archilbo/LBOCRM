...
export function AppGlobalSearch() {
    ...
    useEffect(() => {
        if (isOpen && hasQuery && inputRef.current) {
    ...
    function DropdownPortal() {
        return createPortal(
            <div ref={portalRef} style={dropdownStyle} className="z-[9999] overflow-hidden rounded-3xl border bg-[var(--surface)] shadow-2xl">
                                    ...
                                    <p className="mt-1 truncate text-xs text-[var(--text-muted)]">
                                        ...
                                        {result.archive ? (
                                            <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[10px]">
                                                <span className="rounded-md border border-amber-500/20 bg-amber-500/10 px-1.5 py-0.5 font-medium text-amber-400">
                                                    {result.archive.number}
                                                </span>
                                                <span className="text-[var(--text-muted)]">
                                                    {result.archive.location}
                                                </span>
                                                <span className="capitalize text-[var(--text-muted)]">
                                                    {result.archive.status.replace(/_/g, ' ')}
                                                </span>
                                            </div>
                                        ) : result.type === 'Project' ? (
    ...
    return (
        <div ref={wrapperRef} className="relative w-full max-w-xl">
            ...
            {isOpen && hasQuery ? <DropdownPortal /> : null}
        </div>
    );
}
