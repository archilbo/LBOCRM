...
const INTERMEDIARY_KPI_TONES = {
    ...
    linkedClients: { icon: <IconUserCheck size={16} className="text-violet-400" />, accentColor: '#a78bfa', valueClassName: 'text-violet-300' },
} as const;
...
export default function IntermediariesIndex({ intermediaries, metrics }: PageProps) {
    ...
    function RowMenu({ item }: { item: IntermediaryRow }) {
        ...
        return (
            <div className="flex items-center gap-0.5">
                {item.capabilities.view ? <AppButton isIconOnly compact variant="quiet" size="sm" tooltip={t('intermediaries.viewIntermediary')} aria-label={t('intermediaries.viewIntermediary')} onPress={() => router.visit(`/intermediaries/${item.id}`)} className="size-7 text-[var(--text-muted)]"><IconEye size={13} /></AppButton> : null}
                {item.capabilities.update ? <AppButton isIconOnly compact variant="quiet" size="sm" tooltip={t('intermediaries.editIntermediary')} aria-label={t('intermediaries.editIntermediary')} onPress={() => openEditDrawer(item)} className="size-7 text-[var(--text-muted)]"><IconPencil size={13} /></AppButton> : null}
                {items.length ? <Dropdown>
                    <Dropdown.Trigger className="flex size-7 items-center justify-center rounded text-[var(--text-muted)] transition hover:bg-[var(--surface-2)] data-[open]:text-[var(--accent)]" aria-label={t('intermediaries.actions')}><IconDots size={13} /></Dropdown.Trigger>
                    <Dropdown.Popover placement="bottom end" className="min-w-40 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-1 shadow-xl">
                        <Dropdown.Menu aria-label={t('intermediaries.actions')} onAction={(key) => items.find((menuItem) => menuItem.id === key)?.action()} itemClasses={{ base: 'rounded-lg px-2 py-1 text-[10px] font-medium text-[var(--text)] transition data-[hover]:bg-[var(--danger)]/10' }}>
    ...
    const intermediaryColumns: AppWorkspaceTableColumn<IntermediaryRow>[] = [
        { id: 'select', label: can('intermediaries.delete') ? <Checkbox isSelected={allPageSelected} onChange={togglePageSelection} aria-label={t('intermediaries.selectAll')}><Checkbox.Content><Checkbox.Control className="size-4 rounded border border-[color-mix(in_srgb,var(--text-muted)_35%,transparent)] bg-[var(--surface)]"><Checkbox.Indicator className="text-black" /></Checkbox.Control></Checkbox.Content></Checkbox> : null, headerClassName: 'w-10', cellClassName: 'w-10', fixedPosition: 'start', reorderable: false, render: (item) => item.capabilities.delete ? <span onClick={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}><Checkbox isSelected={selectedIds.has(item.id)} onChange={() => toggleSelection(item.id)} aria-label={t('intermediaries.selectIntermediary', { name: item.name })}><Checkbox.Content><Checkbox.Control className="size-4 rounded border border-[color-mix(in_srgb,var(--text-muted)_35%,transparent)] bg-[var(--surface)]"><Checkbox.Indicator className="text-black" /></Checkbox.Control></Checkbox.Content></Checkbox></span> : null },
        { id: 'avatar', label: '', headerClassName: 'w-8', reorderable: false, render: (item) => <span className={cn('flex size-6 shrink-0 items-center justify-center rounded text-[9px] font-bold', avatarColor(item.id))}>{item.name.charAt(0).toUpperCase()}</span> },
        { id: 'intermediary', label: <ColumnHeader label={t('intermediaries.table.intermediary')} icon={IconBuilding} field="name" />, render: (item) => <div className="min-w-0"><p className="max-w-[180px] truncate font-medium text-[var(--text)]">{item.name}</p><p className="max-w-[180px] truncate text-[var(--text-muted)]">{item.code}</p></div> },
        { id: 'type', label: <ColumnHeader label={t('intermediaries.table.type')} icon={IconUserCircle} field="type" />, render: (item) => <StatusPill label={typeLabel(item.type)} color="primary" size="sm" /> },
        { id: 'contact', label: <ColumnHeader label={t('intermediaries.table.contact')} icon={IconPhone} />, render: (item) => <div className="grid gap-0.5"><span className="text-[var(--text)]">{formatContact(item.phone)}</span><span className="max-w-[160px] truncate text-[10px] text-[var(--text-muted)]">{formatContact(item.email)}</span></div> },
        { id: 'clients', label: <ColumnHeader label={t('intermediaries.table.clients')} icon={IconUsers} field="clientsCount" />, render: (item) => <span className="inline-flex min-w-6 items-center justify-center rounded bg-[var(--surface-2)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--text)]">{item.clientsCount}</span> },
        { id: 'status', label: <ColumnHeader label={t('intermediaries.table.status')} icon={IconCircleCheck} field="isActive" />, render: (item) => <StatusPill label={item.isActive ? t('intermediaries.statuses.active') : t('intermediaries.statuses.inactive')} color={item.isActive ? 'success' : 'warning'} size="sm" /> },
        { id: 'updated', label: <ColumnHeader label={t('intermediaries.table.updated')} icon={IconRefresh} field="updatedAt" />, render: (item) => <span className="whitespace-nowrap text-[var(--text-muted)]">{item.updatedAt || '-'}</span> },
