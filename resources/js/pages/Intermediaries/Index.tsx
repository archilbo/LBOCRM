...
export default function IntermediariesIndex({ intermediaries, metrics }: PageProps) {
    ...
    const intermediaryColumns: AppWorkspaceTableColumn<IntermediaryRow>[] = [
        ...
        { id: 'projects', label: <ColumnHeader label={t('intermediaries.projects')} icon={IconUsers} field="projectsCount" />, render: (item) => <span className="inline-flex min-w-6 items-center justify-center rounded bg-[var(--surface-2)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--text)]">{item.projectsCount}</span> },
