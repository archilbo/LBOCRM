...
export default function IntermediariesIndex({ intermediaries, metrics }: PageProps) {
    ...
    const intermediaryColumns: AppWorkspaceTableColumn<IntermediaryRow>[] = [
        ...
        { id: 'contact', label: <ColumnHeader label={t('intermediaries.table.contact')} icon={Phone} />, render: (item) => <div className="grid gap-0.5"><span className="text-[var(--text)]">{formatContact(item.phone)}</span><span className="max-w-[160px] truncate text-[10px] text-[var(--text-muted)]">{formatContact(item.email)}</span></div> },
        { id: 'clients', label: <ColumnHeader label={t('intermediaries.table.clients')} icon={Users} field="clientsCount" />, render: (item) => <span className="inline-flex min-w-6 items-center justify-center rounded bg-[var(--surface-2)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--text)]">{item.clientsCount}</span> },
