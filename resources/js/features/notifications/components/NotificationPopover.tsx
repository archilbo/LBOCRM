...
function NotifCard({ n, onClose }: { n: EnrichedNotification; onClose: () => void }) {
    ...
    return (
        <button type="button" onClick={handleClick}
            ...
            {!n.isRead ? <span className={`absolute left-0 top-[14px] size-1.5 rounded-full ${sev.dot}`} /> : null}
            ...
            <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[9px] font-semibold ${n.isRead ? 'text-[var(--crm-text-muted)]/50' : sev.dot.replace('bg-', 'text-') + '/80'}`}>
...
export function NotificationPopover() {
    ...
    const { auth } = usePage().props as { auth: { user?: { recent_notifications?: NotificationRow[]; unread_notifications?: number } } };
    const notifRows = auth?.user?.recent_notifications ?? [];
    const unreadCount = auth?.user?.unread_notifications ?? 0;
    ...
    const handleMarkAllRead = useCallback(() => {
        router.post('/notifications/read-all', {}, { preserveScroll: true, onSuccess: () => toast.success('All marked as read.') });
    }, []);
                            ...
                            <div className="flex-1 overflow-y-auto">
                                {filtered.length === 0 ? (
                                    ...
                                    TIME_GROUP_ORDER.map((group) => {
                                        ...
                                        return <NotifGroup key={group} label={group === 'now' ? 'Now' : group === 'today' ? 'Today' : group === 'yesterday' ? 'Yesterday' : 'Earlier'} items={items} onClose={close} />;
                                    })
                                )}
                            </div>
