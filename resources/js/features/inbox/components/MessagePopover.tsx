...
import { Check, ChevronDown, ExternalLink, Filter, Mail, MessageSquare, Search, Trash2, UserRound, Users } from 'lucide-react';
...
const FILTERS = [
    { id: 'recent', label: 'Tous les messages', icon: MessageSquare },
    { id: 'unread', label: 'Non lus', icon: Mail },
    { id: 'direct', label: 'Conversations directes', icon: UserRound },
        ...
        <div className={`group flex items-center border-b border-[color-mix(in_srgb,var(--border)_55%,transparent)] px-2 py-1 last:border-0 ${conversation.unreadCount > 0 ? 'bg-[color-mix(in_srgb,var(--accent)_6%,transparent)]' : ''}`}>
            <Button variant="ghost" onPress={onSelect} className="h-auto min-w-0 flex-1 justify-start rounded-lg px-2 py-2 text-left">
                <div className="relative shrink-0">
                    <Avatar size="sm" name={name} className={`${isGroup ? 'rounded-lg' : ''} ${tone.bg} ${tone.text}`}>{isGroup ? <Users size={15} /> : getConversationInitials(conversation, currentUserId)}</Avatar>
            ...
            <InboxIconButton label="Retirer de la liste" tone="danger" size="sm" onPress={onRemove} className="shrink-0 text-red-400 sm:opacity-70 sm:group-hover:opacity-100"><Trash2 size={13} /></InboxIconButton>
        </div>
    );
}
                ...
                <Popover.Dialog className="outline-none">
                    ...
                    <div className="flex items-center gap-2 border-b border-[var(--border)] px-3 py-2">
                        <SearchField value={search} onChange={setSearch} aria-label="Rechercher dans les messages" variant="secondary" className="min-w-0 flex-1">
                            <SearchField.Group>
                                <SearchField.SearchIcon><Search size={13} /></SearchField.SearchIcon>
                        ...
                        <Dropdown>
                            ...
                            <Dropdown.Popover placement="bottom end" className="min-w-56 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-1 shadow-xl">
                                <Dropdown.Menu selectionMode="single" selectedKeys={[filter]} onAction={(key) => setFilter(String(key) as PopoverFilter)}>
                                    {FILTERS.map((item) => { const Icon = item.icon; return <Dropdown.Item key={item.id} id={item.id} textValue={item.label}><div className="flex items-center gap-2"><Icon size={14} />{item.label}{filter === item.id ? <Check size={12} className="ml-auto text-[var(--accent)]" /> : null}</div></Dropdown.Item>; })}
                                </Dropdown.Menu>
                            </Dropdown.Popover>
                        </Dropdown>
                    </div>
                    ...
                    <footer className="border-t border-[var(--border)] p-2.5">
                        <Button variant="primary" size="sm" onPress={() => visitInbox()} className="w-full"><MessageSquare size={13} /> Ouvrir tous les messages</Button>
                    </footer>
                </Popover.Dialog>
            </Popover.Content>
        </Popover>
