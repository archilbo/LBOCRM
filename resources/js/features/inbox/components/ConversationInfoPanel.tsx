...
export function ConversationInfoPanel({ conversation, messages, currentUserId, onArchiveToggle, collapsed, onToggleCollapsed, mobileOpen = false, onMobileClose, onPreference, onMarkUnread }: Props) {
            ...
            <div className="flex h-full min-h-0 flex-col bg-[var(--surface)] text-[var(--foreground)]">
                ...
                <ScrollShadow className="min-h-0 flex-1 space-y-4 px-4 py-4">
                    <section className="text-center">
                        <Avatar size="lg" className="mx-auto size-16 bg-[var(--accent-soft)] text-lg font-bold text-[var(--accent)]">
                    ...
                    <InfoSection icon={IconUsers} title={t('inbox.membersSection')}>
                        <div className="space-y-1">
                            {participants.map((participant) => {
                                ...
                                return <div key={participant.id} className="flex items-center gap-2.5 rounded-xl px-2 py-2 transition hover:bg-[var(--surface-2)]"><div className="relative"><Avatar size="sm"><Avatar.Fallback>{initials(user?.name || t('inbox.user'))}</Avatar.Fallback></Avatar>{online ? <span className="absolute bottom-0 right-0 size-2.5 rounded-full border-2 border-[var(--surface)] bg-emerald-400" /> : null}</div><span className="min-w-0 flex-1"><span className="block truncate text-xs font-medium">{user?.name || t('inbox.user')}</span><span className="block truncate text-[10px] text-[var(--text-muted)]">{online ? t('inbox.online') : user?.email || t('inbox.offline')}</span></span>{participant.role && participant.role !== 'member' ? <Chip size="sm" variant="soft" className="h-5 text-[9px]">{participant.role}</Chip> : null}</div>;
                            })}
                        </div>
                    </InfoSection>
                    ...
                    {files.length > 0 ? <InfoSection icon={IconFileText} title={t('inbox.filesSection')}><div className="space-y-1.5">{files.map((file) => <a key={file.id} href={file.downloadUrl || file.url || '#'} className="flex h-10 w-full items-center gap-2 rounded-xl bg-[var(--surface-2)] px-2.5 text-xs transition hover:bg-[var(--surface-3)]"><IconFileText size={14} className="shrink-0 text-[var(--accent)]" /><span className="truncate">{file.originalFilename}</span></a>)}</div></InfoSection> : null}

                    {conversation.context && Object.values(conversation.context).some(Boolean) ? <InfoSection icon={IconBriefcase} title={t('inbox.linkedContext')}><div className="grid grid-cols-2 gap-1.5">{conversation.context.clientId ? <a href={`/clients/${conversation.context.clientId}`} className="flex items-center gap-2 rounded-lg bg-[var(--surface-2)] px-2 py-1.5 text-xs transition hover:bg-[var(--surface-3)]"><IconBriefcase size={13} />{t('inbox.client')}</a> : null}{conversation.context.dossierId ? <a href={`/dossiers/${conversation.context.dossierId}`} className="flex items-center gap-2 rounded-lg bg-[var(--surface-2)] px-2 py-1.5 text-xs transition hover:bg-[var(--surface-3)]"><IconFolder size={13} />{t('inbox.dossier')}</a> : null}{conversation.context.taskId ? <a href={`/tasks?task=${conversation.context.taskId}`} className="flex items-center gap-2 rounded-lg bg-[var(--surface-2)] px-2 py-1.5 text-xs transition hover:bg-[var(--surface-3)]"><IconListCheck size={13} />{t('inbox.task')}</a> : null}{conversation.context.financeDocumentId ? <a href={`/finance/documents/${conversation.context.financeDocumentId}`} className="flex items-center gap-2 rounded-lg bg-[var(--surface-2)] px-2 py-1.5 text-xs transition hover:bg-[var(--surface-3)]"><IconReceipt2 size={13} />{t('inbox.finance')}</a> : null}</div></InfoSection> : null}
                </ScrollShadow>
    ...
    return <><aside className={`hidden shrink-0 border-l border-[var(--border)] bg-[var(--surface)] xl:flex ${collapsed ? 'w-12 items-start justify-center' : 'w-[288px]'}`}>{collapsed ? <div className="space-y-3 py-3"><InboxIconButton label={t('inbox.showDetails')} tone="accent" onPress={onToggleCollapsed} className="border border-[var(--border)]"><IconLayoutSidebarRightExpand size={15} /></InboxIconButton>{conversation ? <Avatar size="sm"><Avatar.Fallback>{conversationInitial(conversation, currentUserId)}</Avatar.Fallback></Avatar> : null}</div> : <div className="relative h-full min-h-0 w-full"><InboxIconButton label={t('inbox.hideDetails')} onPress={onToggleCollapsed} className="absolute right-3 top-3 z-10 border border-[var(--border)] bg-[var(--surface)]"><IconLayoutSidebarRightCollapse size={15} /></InboxIconButton>{panel || <div className="flex h-full items-center justify-center p-8 text-center text-sm text-[var(--text-muted)]">{t('inbox.selectConversation')}</div>}</div>}</aside><AppDrawer isOpen={mobileOpen} onOpenChange={(open) => { if (!open) onMobileClose?.(); }} hideHeader isDismissable panelClassName="xl:hidden !w-[min(100vw,384px)]" contentClassName="!p-0">{panel}</AppDrawer></>;
}
