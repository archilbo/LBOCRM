...
const inboxTabs = [
    { id: 'active', label: 'Tous les messages', icon: IconMessage2 },
    { id: 'unread', label: 'Non lues', icon: IconCircleDot },
    { id: 'direct', label: 'Messages directs', icon: IconMessageCircle },
    { id: 'groups', label: 'Groupes', icon: IconUsers },
    { id: 'archived', label: 'Archivees', icon: IconArchive },
];
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                        ...
                        {conversation.isPinned ? <IconBookmark size={11} className="shrink-0 fill-[var(--accent)] text-[var(--accent)]" aria-label="Conversation epinglee" /> : null}
        ...
        <aside className="flex h-full min-h-0 flex-col bg-[var(--surface)]">
            <header className="border-b border-[var(--border)] px-3 pb-3 pt-3">
                <div className="flex items-center gap-3">
                    ...
                    {onNewConversation ? <InboxIconButton label="Nouvelle conversation" tone="accent" onPress={onNewConversation} className="bg-[var(--accent-soft)]"><IconPlus size={17} /></InboxIconButton> : null}
                </div>
                <div className="mt-3 flex gap-1.5">
                    <Input aria-label="Rechercher une conversation" value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="Rechercher..." size="sm" startContent={<IconSearch size={13} />} className="min-w-0 flex-1" classNames={{ inputWrapper: 'h-8 rounded-lg border border-[var(--border)] bg-[var(--surface-2)] shadow-none data-[focus-visible=true]:border-[var(--accent)]', input: 'text-[10px] text-[var(--foreground)] placeholder:text-[var(--text-muted)]' }} />
                    <Dropdown>
                        <Dropdown.Trigger aria-label="Filtrer les conversations" className={`flex size-8 shrink-0 items-center justify-center rounded-lg border transition ${activeTab !== 'active' ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]' : 'border-[var(--border)] bg-[var(--surface-2)] text-[var(--text-muted)]'}`}><IconAdjustmentsHorizontal size={13} /></Dropdown.Trigger>
