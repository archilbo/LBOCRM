...
import { AlertTriangle, CheckCircle2, Clock3, ListTodo, RefreshCw, TimerOff, UserMinus } from 'lucide-react';
...
export function UserOperationsReportTab({ report, reportedAt, onRefresh }: { report: OperationsReport; reportedAt: string; onRefresh: () => void }) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-[11px] text-[var(--text-muted)]">Données consolidées au {new Intl.DateTimeFormat('fr-MA', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(reportedAt))}.</p><AppButton variant="quiet" compact onPress={onRefresh}><RefreshCw size={13} />Actualiser</AppButton></div>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                <AppKpiCard label="Total" value={report.totalTasks} detail="Tâches créées" icon={<ListTodo size={15} className="text-[var(--crm-info)]" />} valueClassName="text-[var(--crm-info)]" accentColor="var(--crm-info)" />
                <AppKpiCard label="Ouvertes" value={report.openTasks} detail={`${report.dueThisWeek} cette semaine`} icon={<Clock3 size={15} className="text-[var(--crm-gold)]" />} valueClassName="text-[var(--crm-gold)]" accentColor="var(--crm-gold)" />
                <AppKpiCard label="Terminées" value={report.completedTasks} detail={`${completionRate}% réalisées`} icon={<CheckCircle2 size={15} className="text-[var(--crm-success)]" />} valueClassName="text-[var(--crm-success)]" accentColor="var(--crm-success)" />
                <AppKpiCard label="Bloquées" value={report.blockedTasks} detail={`${report.unassignedTasks} sans responsable`} icon={<AlertTriangle size={15} className="text-[var(--crm-danger)]" />} valueClassName="text-[var(--crm-danger)]" accentColor="var(--crm-danger)" />
                <AppKpiCard label="En retard" value={report.overdueTasks} detail="À replanifier" icon={<TimerOff size={15} className="text-[var(--crm-violet)]" />} valueClassName="text-[var(--crm-violet)]" accentColor="var(--crm-violet)" />
            </div>
            ...
            <AppCard className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-semibold text-[var(--text)]">Avancement global</p><p className="mt-0.5 text-[11px] text-[var(--text-muted)]">{report.completedTasks} tâche(s) terminée(s) sur {report.totalTasks}.</p></div><div className="flex min-w-52 flex-1 items-center gap-3 sm:max-w-md"><div className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--surface-2)]"><div className="h-full rounded-full bg-[var(--crm-success)] transition-[width] duration-300" style={{ width: `${completionRate}%` }} /></div><Chip size="sm" variant="soft" color="success">{completionRate}%</Chip></div>{report.unassignedTasks > 0 ? <Chip size="sm" variant="soft" color="warning" startContent={<UserMinus size={12} />}>{report.unassignedTasks} sans responsable</Chip> : null}</AppCard>
        </div>
    );
}
