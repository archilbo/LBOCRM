...
export function createFinanceDocumentActions(
    ...
    const actions: Array<FinanceRowAction | false> = [
        ...
        !document.isInternal && can('finance.view') && { id: 'preview', label: translate('finance.actions.preview'), icon: <IconFileText size={13} />, onPress: () => handlers.onPreview(document), dividerBefore: true },
        !document.isInternal && can('finance.view') && { id: 'print', label: translate('finance.actions.print'), icon: <IconPrinter size={13} />, onPress: () => handlers.onPrint(document) },
        !document.isInternal && can('finance.view') && Boolean(document.hasPdf) && { id: 'download-pdf', label: translate('finance.actions.downloadPdf'), icon: <IconDownload size={13} />, onPress: () => handlers.onDownloadPdf(document) },
        !document.isInternal && can('finance.view') && Boolean(document.hasExcel) && { id: 'download-excel', label: translate('finance.actions.downloadExcel'), icon: <IconFileSpreadsheet size={13} />, onPress: () => handlers.onDownloadExcel(document) },
        !document.isInternal && can('finance.documents.update') && { id: 'generate-pdf', label: translate(document.hasPdf ? 'finance.actions.regeneratePdf' : 'finance.actions.generatePdf'), icon: <IconFileText size={13} />, onPress: () => handlers.onGeneratePdf(document), tone: 'accent', dividerBefore: true },
        !document.isInternal && can('finance.documents.update') && { id: 'generate-excel', label: translate(document.hasExcel ? 'finance.actions.regenerateExcel' : 'finance.actions.generateExcel'), icon: <IconFileSpreadsheet size={13} />, onPress: () => handlers.onGenerateExcel(document), tone: 'accent' },
        ...
        can('finance.documents.create') && (document.type === 'quote' || document.type === 'internal_invoice') && Boolean(document.convertToInvoiceUrl) && { id: 'convert', label: document.type === 'internal_invoice' ? 'Convertir en facture officielle' : translate('finance.actions.convertToInvoice'), icon: <IconRefresh size={13} />, onPress: () => handlers.onConvert(document), tone: 'accent' },
        ...
        can('finance.payments.create') && (document.type === 'invoice' || document.type === 'internal_invoice') && Boolean(document.paymentUrl) && { id: 'payment', label: translate('finance.actions.registerPayment'), icon: <IconWallet size={13} />, onPress: () => handlers.onPayment(document), tone: 'success', dividerBefore: true },
